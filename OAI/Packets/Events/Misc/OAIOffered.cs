...
namespace OAI.Packets.Events.Misc
{
     ...
     * Offered – OF
     * 
     * Because the application must respond to the Offered event, 
     * Call Processing slows down at the device. If an application 
     * is always going to “accept” calls at the device, offering control
     * should be terminated.
     * 
     * Allows an application to redirect calls that arrive at a 
     * specific station (phone) before the call rings the station 
     * or follows any programmed forwards or DND. When a call 
     * arrives at the controlled station device, the communications 
     * system generates this event to allow the application to 
     * change the call’s destination before the station device 
     * rings. An application must gain offering control over the 
     * station to enable this mode, and only one application can 
     * have offering control at each station.
     * 
     * OF,<Resync_Code>,<Mon_Cross_Ref_ID>,<Call_ID>,<Alerting_Internal_Ext>,
     ...
     * <Local_Cnx_State>,<Event_Cause>,<Hunt_Group_ID>
     * 
     */
    public class OAIOffered : OAIMisc
    {
        public const string EVENT = "OF";

        protected OAICallModel CallModel;

        public OAIOffered(string[] parts) : base(parts) { }
        public OAIOffered(byte[] bytes) : base(bytes) { }
